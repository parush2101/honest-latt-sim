...
of post-treatment parallel-trends violation that would just overturn the stated conclusion. It is judged
against the violations the data leave visible, the retained cohorts' own pre-trends, which the screen holds
below $c$ and which are measured in the same units, and against the effect at stake, so that an $M^\ast$
several times the credible cohorts' visible pre-trends signals a robust conclusion. The application of
Section~\ref{sec:app} reads $M^\ast$ against both benchmarks.
...
deterministic and inference behaves conventionally. A researcher unwilling to assume separation has two
routes. One buys validity through independence between selection and estimation, selecting on
pre-determined covariates or an independent sample split, at the cost of the efficiency that split data
forgoes. The other conditions on the selection rather than avoiding it, and recovers that efficiency; we
develop it now, because it delivers the uniform guarantee that Proposition~\ref{prop:oracle} lacks.

The construction randomizes the screen and conditions on its outcome. Working in the reduced-form model
...
estimating the selected aggregate $\theta_S=\ell_S'\beta_{\mathrm{post}}$ from the unrandomized
$\hat\theta_S=\ell_S'\hat\beta_{\mathrm{post}}$, where $\ell_S$ carries the reweighting onto $S$. The
target is the reduced-form aggregate: under parallel trends for $S$ it is the causal $\theta_S$, and in
general it is that quantity plus the residual violation the level bound accounts for below, so selection
uncertainty and identification are handled by separate instruments that compose.
...
uniform guarantee, exactly across the local-to-threshold configurations that obstruct it, and it does so
for the sampling and selection uncertainty in the reduced-form aggregate; the identification gap remains
the province of the level bound, so the honest causal interval is the carved interval widened by $M$,
$[\,\inf\mathcal C_{1-\alpha}(\hat S)-M,\ \sup\mathcal C_{1-\alpha}(\hat S)+M\,]$, valid whenever the
retained set's residual violation lies in $\Delta^{\mathrm{Level}}(M)$. Simulations in the reduced-form
model bear this out. With one pre-period per cohort and a pre--post sampling correlation of $0.7$, the
naive post-selection interval covers at $0.92$ and then $0.78$ as the retained cohort's population
pre-trend moves to one and two thresholds off center, the local-to-threshold regime, whereas the carved
interval holds between $0.93$ and $0.96$ across every configuration. The randomization buys back power a
split would forfeit: at the most severe configuration the carved interval at $\gamma=0.5$ is roughly a
quarter narrower than the fully conditional one at $\gamma=0$, at unchanged coverage.
...
The same accounting extends to $K$ cohorts, and the two-cohort ledger turns out to be
...
The general threshold \eqref{eq:generalK-threshold} is the two-cohort ledger written for many
cohorts, with each of its three forces now an aggregate. The reason to trade is the total violation
$B_{\mathcal F}$ that selection removes, the squared sum of the dropped cohorts' weighted violations,
not any single one. The estimand cost is the composition gap $\Gamma$, the difference between the
retained subpopulation's effect and the full population's, which is the many-cohort form of the
heterogeneity $\gamma$ and vanishes under the equal-targets condition of
Proposition~\ref{prop:coincide}. And the variance cost $\Delta\mathrm{Var}$ is the increase in
sampling variance from reweighting onto fewer cohorts. The qualitative reading of the two-cohort
case therefore holds at any cohort count: the sign of the advantage is a violation-against-noise-and-
heterogeneity question, settled by \eqref{eq:generalK-threshold} and independent of how informative
the pre-trends are, while informativeness governs only magnitude, since $\Delta$ runs from zero when
the screen retains everyone, $\pi_g=1$, to $\Delta^\ast$ when it separates the cohorts. Two
