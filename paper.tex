...
point-identified precisely when the ATT is not. It is estimated by reweighting standard group-time
effects toward those cohorts, and paired with honest sensitivity bounds on the residual violation of the
...
strict $c$ the reverse. Because a cohort's pre- and post-treatment coefficient errors are correlated in finite
...
is given the same true treatment effect, normalized to one. This normalization is deliberate, making
...
As precision improves the LATT's bias falls monotonically from $+0.015$ to zero, and this residual is the
...
paper's basic point in miniature, the LATT facing a noise problem, which more data cures, whereas the
...
Event study, all cohorts (pooled ATT) versus the credible subpopulation (LATT) at the illustrative
threshold $c\approx0.03$, with $95\%$ bands. (c) The credibility path, the LATT as the screen threshold
$c$ is relaxed, near zero across the flat-pre-trend cohorts and rising toward the pooled ATT only as
